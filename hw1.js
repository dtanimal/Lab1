 ...
 * *****************
 *
 * Implement the Circle class below.
 *
 * It needs to have a radius(Number) property and a color(String) properties. Both properties should be set in a constructor.
 *
...
const Product = class Product {
  ...
  static halfOff = (products) => {
    return products.map((prod) => ({ ...prod, price: prod.price * 0.5 }));
  };
  ...
  static printProducts = (products) => {
    products.forEach((element) => {
      console.log(
        `Product: ${element.name}, Cost: $${element.price.toFixed(
          2
        )}, Availability: ${element.availability === "In Stock" ? "Yes" : "No"}`
      );
    });
  };
};
